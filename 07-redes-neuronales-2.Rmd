...
```{r}
library(tensorflow)
# estas líneas puedes no necesitrse
...
# estas son imágenes sintéticas tomadas de
# https://objectcomputing.com/resources/publications/sett/june-2019-using-machine-learning-to-read-analog-gauges
imagenes_paths <- list.files("./datos/gauges/", full.names = TRUE)
imagenes <- map(imagenes_paths, ~ image_load(.x, target_size = c(80, 80),
                                             grayscale = TRUE))
# Simulamos que tenemos 147*20 ~ 3000 imagenes para entrenar
imagenes <- rep(imagenes, 20)
y <- map(imagenes_paths, ~ str_extract_all(.x, "[0-9]+")[[1]]) %>% 
  map_chr(~ paste0(.x, collapse = ".")) %>% 
  as.numeric()
y <- rep(y, 20)
```

El siguiente generador lo utizaremos para crear imágenes con las que entrenamos
y validamos. Ojo: usualmente, el generador lo usamos para aumentar los datos
entrenamiento y mejorar el desempeño en validación (normalmente en validación no
hacemos rotaciones o acercamientos, por ejemplo):
...
imgs_array <- map(imgs_array, ~ array_reshape(.x, c(1, 80, 80, 1)))
...
generador_validacion <- flow_images_from_data(
  x = x,
  y = matrix(y, length(y), 1),
  generator = generador_1,
  shuffle = TRUE,
...
Mostramos algunos ejemplos del proceso generador de datos sintético que
acabamos de crear:
...
modelo <- keras_model_sequential() %>% 
  layer_conv_2d(input_shape = c(80, 80, 1), filters = 16, kernel_size = c(7, 7),
  ...
  layer_conv_2d(filters = 8, kernel_size = c(3, 3), activation = 'relu',
                name = "conv_3") %>%
  layer_max_pooling_2d(pool_size = c(2,2), name = "conv_3_down") %>% 
  layer_dropout(rate = 0.1) %>% 
  layer_flatten() %>%  
  layer_dropout(rate = 0.1) %>% 
  layer_dense(units = 20, activation = "relu", kernel_regularizer = regularizer_l2(0.0001),
              name = "densa_1") %>% 
  layer_dropout(rate = 0.2) %>% 
  layer_dense(units = 1, activation = "linear", kernel_regularizer = regularizer_l2(0.0001))
modelo %>%
  compile(
    loss = "mse",
...
**Nota**: en este ejemplo, también usamos [dropout](https://www.cs.toronto.edu/~hinton/absps/JMLRdropout.pdf)
para evitar sobreajustar la red. Generalmente se aplica a capas totalmente conexas, y funciona
eliminando unidades al azar durante el entrenamiento. Esto reduce la correlacción entre
unidades, y promueve que cada unidad aprenda cosas útiles sin depender mucho de otras.


```{r}
val_datos <- generator_next(generador_validacion)
history <- modelo %>% fit(
  x = datos_entrena[[1]],
  y = datos_entrena[[2]],
  epochs = 800,
  batch_size = 64,
  verbose = TRUE,
  validation_data = val_datos)
```

Nótese que en este caso el error de validación es ligeramente más bajo que el 
de entrenamiento: esto se debe en este ejemplo tenemos poco sobreajuste, y a
que los errores de validación incluyen el dropout (promediando
resultados de cada minilote),
mientras que en validación usamos la red completa.
...
```{r}
prueba_datos <- generator_next(generador_validacion)

score <- modelo %>% evaluate(prueba_datos[[1]], prueba_datos[[2]])
score 
```

```{r}
preds <- predict(modelo, prueba_datos[[1]]) %>% as.numeric()
tibble(y = prueba_datos[[2]], .pred = preds) %>% 
...
Vamos ahora ver las activaciones de la primera capa para algunas imágenes. Las
...
red_conv_1 <- keras_model(inputs = modelo$input,
    outputs = get_layer(modelo, 'conv_1_down')$output)
gen_nuevo <- generator_next(generador_validacion)
x_nuevo <- gen_nuevo[[1]]
y_nuevo <- gen_nuevo[[2]]
x_nuevo <- x_nuevo[order(y_nuevo[,1]),,,,drop = FALSE]
y_nuevo <- y_nuevo[order(y_nuevo[,1]),1]

activaciones_1 <- predict(red_conv_1, x_nuevo)
```
...
graficar_activaciones <- function(activaciones, ind, ncol = 1, filtros = NULL){
  act_ind <- activaciones[ind,,,] # drop primera dimensión
  imagen <- x_nuevo[ind,,,1]
  x_tamaño <- dim(act_ind)[2]
  y_tamaño <- dim(act_ind)[1]
  num_filtros <- dim(act_ind)[3]
  digito_df <- tibble(val = as.numeric(imagen), pixel = 1:(80*80)) %>% 
      mutate(y = (pixel-1) %% 80, x = (pixel-1) %/% 80)
  unidades_lista <- lapply(1:dim(act_ind)[3], function(i){
    mat <- act_ind[,,i]
  ...
  g_1 <- ggplot(dat, aes(x = x, y = -y, fill = val)) + geom_raster() + 
    ...
    scale_fill_gradient2(low = "red", mid = "red", high = "white", midpoint = 0.0) +
    coord_equal() +
...
graficar_activaciones(activaciones_1, 1, 4) / graficar_activaciones(activaciones_1, 201, 4)
```


Veamos ahora las activaciones de las última capa convolucional para distintas
imágenes. Nótese que varias de las unidades de estas imágenes están dedicadas a ubicar
la posición de la aguja, y que igualmente algunas extraen también la posición
de la base del indicador (los tornillos y la leyenda):

```{r}
red_conv_2 <- keras_model(inputs = modelo$input,
    outputs = get_layer(modelo, "conv_3_down")$output)
activaciones_2 <- predict(red_conv_2, x_nuevo)
dim(activaciones_2)
```
...
ncol <- 8
graficar_activaciones(activaciones_2, 1, ncol) / graficar_activaciones(activaciones_2, 4, ncol) / graficar_activaciones(activaciones_2, 100, ncol)/ graficar_activaciones(activaciones_2, 101, ncol) /
graficar_activaciones(activaciones_2, 150, ncol)/ graficar_activaciones(activaciones_2, 152, ncol) /
  graficar_activaciones(activaciones_2, 200, ncol)/ graficar_activaciones(activaciones_2, 199, ncol) /
  graficar_activaciones(activaciones_2, 350, ncol)/ graficar_activaciones(activaciones_2, 348, ncol) 
```

Finalmente, la capa totalmente conexa combina todos los pixeles de estas 8 imágenes
filtradas. Sus activaciones son:



```{r}
red_densa <- keras_model(inputs = modelo$input,
    outputs = get_layer(modelo, "densa_1")$output)
activaciones_3 <- predict(red_densa, x_nuevo)
dim(activaciones_3)
activaciones_3_tbl <- activaciones_3 %>% 
  as_tibble() %>% 
  mutate(ind = row_number())
loadings_1 <- princomp(activaciones_3_tbl %>% select(-ind))$loadings[,1]
orden_1 <- loadings_1 %>% order()
activaciones_3_tbl <- activaciones_3[,orden_1] %>% 
  as_tibble() %>% 
  mutate(ind = row_number()) %>% 
  pivot_longer(cols = -ind, names_to = "unidad", values_to = "activacion") %>% 
  mutate(unidad = str_sub(unidad, start = 2) %>% as.integer())
ggplot(activaciones_3_tbl , aes(x = unidad, y = ind, fill = activacion)) +
  geom_tile() + scale_fill_gradient2(low="white", high="red", mid = "white", midpoint = 0.0)
```


```{r}
get_weights(modelo)[[9]][orden_1,] %>% round(3)
```
