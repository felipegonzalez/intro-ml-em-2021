...
que queremos predecir.  Cada vez que obervamos un dato $(x, y)$ lo extraemos
de manera independiente de estas dos distribuciones, forma que supondremos
que el conjunto de entrenamiento

$${\mathcal L} =  (x^{(1)}, y^{(1)}), (x^{(2)}, y^{(2)}), \ldots, (x^{(N)}, y^{(N)}) $$

en una muestra (observaciones independientes e ideńticamente distribuidas) del
...
```{block2, type="resumen"}
El desempeño de un modelo predictivo puede ser grande por dos razones, que pueden
...
**Discusión**: Explica por qué para nuestro ejemplo simple simulado, 1-vecino más cercano resulta
en sobreajuste, y el modelo lineal subajuste. Explica por qué en dimensión alta métodos
locales flexibles pueden producir tanto sesgo alto como varianza alta. Explica sin embargo
como en el caso de que el modelo lineal es apropiado, puede ser que el sesgo y la varianza sean
bajas aún cuando la dimensión sea alta.


## Ejercicio

Usa 5 o 10 vecinos más cercanos y repite los ejercicios que acabamos de hacer con regresión
lineal y 1 vecino más cercano. Discute en términos de la teoría de varianza y sesgo por qué
obtienes mejor o peor desempeño predictivo.
