...
entrena_casas_tbl <- training(casas_split)
receta_casas <- recipe(precio_miles ~ ., data = entrena_casas_tbl) %>% 
  step_filter(condicion_venta == "Normal") %>% 
  step_select(-condicion_venta, -id) %>% 
...
```{r, message = FALSE}
y_entrena <- juice(receta_casas) %>% pull(precio_miles)
x_entrena <- juice(receta_casas) %>% select(-precio_miles) %>% as.matrix
dim(x_entrena)
...
mod_boost_cv <- xgb.cv(params = params, 
                     data = d_entrena,
                     nfold = 10, 
                     nrounds = 1600, # número de árboles
                     print_every_n = 100)
```
...
res <- ajustar_mod(d_entrena, verbose = 1, nrounds = 1600, params = params)
```
...
df_params <- list(objective = "reg:squarederror", 
                  eta = c(0.005, 0.01, 0.05, 0.1, 0.25),
                  subsample = c(0.1, 0.5, 1.0), 
                  colsample_bynode = 0.2,
                  maxdepth = 1, lambda = 0) %>% cross_df
df_params <- df_params %>% 
    mutate(eval_log = pmap(df_params, ~ ajustar_mod(d_entrena, nrounds = 500, params = list(...))))
...
g_1 <- ggplot(df_params_u %>% filter(iter > 5), 
     ...
     geom_hline(yintercept = 25) + geom_line()  
g_1
```
```{r}
df_params_u %>% filter(iter == 500, tipo == "test", eta < 0.5) %>% 
...
params = list(
    objective = "reg:squarederror",
    eta = 0.005, # tamaño de paso
    max_depth = 3, # profundidad de árboles
    min_child_weight = 1,
    colsample_bynode = 0.2,
    subsample = 0.1, 
    lambda = 25)
res <- ajustar_mod(d_entrena, verbose = 1, nrounds = 20000, params = params, 
                   every_n = 500)
```
