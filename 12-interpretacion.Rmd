...
```{r, message = FALSE}
library(pdp)
pd_temp_lm <- pdp::partial(mod_lineal_o, train = air_tbl,  
                           pred.var = "Temp", plot = FALSE) %>% as_tibble()
pd_temp_bo <- pdp::partial(mod_bosque_o, train = air_tbl,  
                           pred.var = "Temp", plot = FALSE) %>% as_tibble()
pd_grafs <- pd_temp_lm %>% 
...
```{r}
pd_temp_lm <- pdp::partial(mod_lineal_o, train = air_tbl,  pred.var = c("Temp", "Wind"), plot = FALSE) %>% as_tibble()
pd_temp_bo <- pdp::partial(mod_bosque_o, train = air_tbl,  pred.var = c("Temp","Wind"), plot = FALSE) %>% as_tibble()
...
error debidos a la variación en la muestra de prueba. 
